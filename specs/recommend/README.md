...
- `source` 只允许 `hot/new_content/interest`。
...
- 快照命中率：`snapshot_hit / snapshot_lookup`。
- 实验 CTR/IPM：来自 `zfeed_rec_metric_daily` 或 Kafka 聚合 exporter。
...
- 2026-06-15 收尾同步点：本轮已停在 user-action 迁移观测能力补齐，生产侧 outbox 和消费侧 consumer 均已有低基数指标；后续不继续追加实现，先进入运行观察和数据校验。
...
- 画像更新已有 `ApplyProfileEvent`，推荐埋点入口已接入 click/dwell/like/favorite/comment/unlike/unfavorite，`zfeed-rec-track` consumer 也能异步更新画像；interaction-rpc `like/cancel_like`、`favorite/remove_favorite`、`comment` 原始事件和统一 user-action JSON 均已兼容，`content-rpc` 也能独立消费 `zfeed-user-action` 并记录消费结果指标，interaction-rpc 侧统一 outbox/producer 基础设施已就绪且已补 outbox 发送/回放指标，点赞/取消点赞、收藏/取消收藏、评论写路径均已接入，front-api 同步互动埋点禁用开关已默认启用。当前文档已同步至 user-action 迁移观测补齐节点；后续需要观察迁移后的 `zfeed-user-action` 消费、画像增量和日聚合数据，再清理 front-api 兼容投递路径。
